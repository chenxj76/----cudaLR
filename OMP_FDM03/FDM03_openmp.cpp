...
/* 200*200 change to 1000*1000 ,so I had deleted 50times of the gate d, f and X */
/*xingji chen,2017/11/18 */
...
int main(int argc, char* argv[])
{
	...
	int nstep = 10 / dt_max; // snapshot interval 10 ms to save data files 
...
#pragma omp parallel private(ncount,tid)// create and destruction of threads takes time, so it's a good place to create all threads here!!
	...
	for (ncount = 0; ncount <= 22/dt_max; ncount++){//simulation time is 10ms
	  if (tid == 0){
		for (i = 1; i < nx + 1; i++){
