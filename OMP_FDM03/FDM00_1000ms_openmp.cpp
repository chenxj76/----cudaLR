/* Method 0 for 2D LR91 */
/* OpenMP is used to speed up, 5-point centered finite difference, not include operator splitting and ADI method. */
...
#pragma omp parallel private(ncount,tid)// create and destruction of threads takes time, so it's a good place to create all threads here!!
	{		
		...
		for (ncount = 0; ncount <= 100 / dt; ncount++){//simulation time is 1000ms
