...
__global__ void comp_ina(double *d_V, double *d_m, double *d_h, double *d_jj, 
		double *d_m0, double *d_h0, double *d_jj0, double *d_dt, double *ina) {
...
__global__ void comp_ical(double *d_V, double *d_d, double *d_f, double *d_d0, 
						double *d_f0, double *d_cai, double *d_dt, 
						double *isi,double *esi){
	int k = threadIdx.x + blockIdx.x * blockDim.x;
	//int i = threadIdx.x;
	...
	//__shared__ double isi[tpb];
        ...
        //double dcai = -0.0001*isi[k] + 0.07*(0.0001 - d_cai[k]);
...
__global__ void update_cai( double *d_cai, double *d_dt, double *isi){
	int k = threadIdx.x + blockIdx.x * blockDim.x;
	//int i = threadIdx.x;
...
void gpu_update_cai(){
	...
	//tpb = 256;	
    bpg = (nx*ny+tpb-1)/tpb;
	update_cai<<<bpg, tpb>>>(d_cai,d_dt,isi);
	cudaDeviceSynchronize();
}

__global__ void comp_ik(double *d_V, double *d_X, double *d_X0, double *d_dt,double *ik){
	int k = threadIdx.x + blockIdx.x * blockDim.x;
	...
	int j = (int)(k/nx);
        ...
        if (d_V[k+nx+2+1+2*j] > -100.0) {
                Xi = 2.837*(exp(0.04*(d_V[k+nx+2+1+2*j] + 77.0)) - 1.0)/
...
__global__ void comp_ik1(double *d_V, double *ik1) {
	int k = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void comp_ikp(double *d_V, double *ikp) {
	int k = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void comp_ib(double *d_V, double *d_it,double *ib,
						double *ik,double *ik1,double *ikp,
						double *isi,double *ina) {
	int k = threadIdx.x + blockIdx.x * blockDim.x;
	...
	int j = (int)(k/nx);
		...
		d_it[k]=ib[k]+ikp[k]+ik1[k]+ik[k]+isi[k]+ina[k];
	}
}
...
void gpu_ion(){
	...
	//tpb = 256;	
    bpg = (nx*ny+tpb-1)/tpb;
	comp_ina<<<bpg, tpb>>>(d_V, d_m, d_h, d_jj, d_m0, d_h0, d_jj0, d_dt,ina);
	comp_ical<<<bpg, tpb>>>(d_V, d_d, d_f, d_d0, d_f0,d_cai, d_dt, isi,esi);
	comp_ik<<<bpg, tpb>>>(d_V, d_X,  d_X0, d_dt,ik);
	comp_ik1<<<bpg, tpb>>>(d_V, ik1);
	comp_ikp<<<bpg, tpb>>>(d_V, ikp);
	comp_ib<<<bpg, tpb>>>(d_V, d_it,ib,ik,ik1,ikp,isi,ina);
	cudaDeviceSynchronize();
}
