...
void Send_to_Host(){
	cudaError_t Error;
        ...
        if (Error != cudaSuccess)
        printf("CUDA error(copy h_dVdt->d_dVdt) = %s\n",cudaGetErrorString(Error));
		...
		Error = cudaMemcpy(h_m0, d_m0, size, cudaMemcpyDeviceToHost);
        if (Error != cudaSuccess)
        printf("CUDA error(copy d_m0->h_m0) = %s\n",cudaGetErrorString(Error));
        Error = cudaMemcpy(h_h0, d_h0, size, cudaMemcpyDeviceToHost);
        if (Error != cudaSuccess)
        printf("CUDA error(copy d_h0->h_h0) = %s\n",cudaGetErrorString(Error));
        Error = cudaMemcpy(h_jj0, d_jj0, size, cudaMemcpyDeviceToHost);
        if (Error != cudaSuccess)
        printf("CUDA error(copy d_jj0->h_jj0) = %s\n",cudaGetErrorString(Error));
        Error = cudaMemcpy(h_d0, d_d0, size, cudaMemcpyDeviceToHost);
        if (Error != cudaSuccess)
        printf("CUDA error(copy d_d0->h_d0) = %s\n",cudaGetErrorString(Error));
        Error = cudaMemcpy(h_f0, d_f0, size, cudaMemcpyDeviceToHost);
        if (Error != cudaSuccess)
        printf("CUDA error(copy d_f0->h_f0) = %s\n",cudaGetErrorString(Error));
        Error = cudaMemcpy(h_X0, d_X0, size, cudaMemcpyDeviceToHost);
        if (Error != cudaSuccess)
        printf("CUDA error(copy d_X0->h_X0) = %s\n",cudaGetErrorString(Error));
