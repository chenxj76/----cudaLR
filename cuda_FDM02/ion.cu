...
__global__ void comp_ina(double *d_V, double *d_m, double *d_h, double *d_jj, double *d_it) {
	...
	double ah, bh, aj ,bj;
        if (d_V[k+nx+2+1+2*j] < -40) {
                ...
                ah = 0;
                bh = 1 / (0.13*(1.0 + exp((d_V[k+nx+2+1+2*j] + 10.66) / -11.1)));
                aj = 0;
        ...
        d_m[k] = mss - (mss - d_m[k])*exp(-dt_max / mtau);
        d_h[k] = hss - (hss - d_h[k])*exp(-dt_max / htau);
        d_jj[k] = jss - (jss - d_jj[k])*exp(-dt_max / jtau);
...
__global__ void comp_ical(double *d_V, double *d_d, double *d_f, double *d_cai, double *d_it,double *isi,double *esi){
	int k = threadIdx.x + blockIdx.x * blockDim.x;
	//int i = threadIdx.x;
	...
	//__shared__ double isi[tpb];
        ...
        double ad = 0.095*exp(-0.01*(d_V[k+nx+2+1+2*j] - 5.0)) / (1 + exp(-0.072*(d_V[k+nx+2+1+2*j] - 5)));
        double bd = 0.07*exp(-0.017*(d_V[k+nx+2+1+2*j] + 44.0)) / (1 + exp(0.05*(d_V[k+nx+2+1+2*j] + 44)));
        double af = 0.012*exp(-0.008*(d_V[k+nx+2+1+2*j] + 28.0)) / (1 + exp(0.15*(d_V[k+nx+2+1+2*j] + 28)));
        double bf = 0.0065*exp(-0.02*(d_V[k+nx+2+1+2*j] + 30.0)) / (1 + exp(-0.2*(d_V[k+nx+2+1+2*j] + 30)));
        ...
        d_d[k] = dss - (dss - d_d[k])*exp(-dt_max / taud);
        d_f[k] = fss - (fss - d_f[k])*exp(-dt_max / tauf);

        isi[k] = 0.09*d_d[k] * d_f[k] * (d_V[k+nx+2+1+2*j] - esi[k]);

        //double dcai = -0.0001*isi[i] + 0.07*(0.0001 - d_cai[k]);

        //d_cai[k] = d_cai[k] + dcai * dt_max;
		d_it[k] = d_it[k] + isi[k];
...
__global__ void comp_ik(double *d_V, double *d_X, double *d_it){
	...
	int j = (int)(k/nx);
        ...
        double taux = 1/ (ax + bx);
        double xss = ax*taux;
        d_X[k] = xss - (xss - d_X[k])*exp(-dt_max / taux);
...
void gpu_Ion(){
	...
	comp_ical<<<bpg, tpb>>>(d_V, d_d, d_f, d_cai, d_it,isi,esi);
