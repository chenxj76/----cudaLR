...
__global__ void comp_dV2(double *d_V ,double *d_dVdt ){
	...
	d_dVdt[k] =D*((d_V[id+1] + d_V[id-1] - 2*d_V[id])
...
void gpu_dV2(){
	...
	//tpb = 256;
    bpg = (nx*ny+tpb-1)/tpb;
	comp_dV2<<<bpg, tpb>>>(d_V, d_dVdt);
	cudaDeviceSynchronize();
}
__global__ void comp_dV2it(double *d_it ,double *d_dVdt ){
	int k = threadIdx.x + blockIdx.x * blockDim.x;
...
void gpu_dV2it(){
	...
	//tpb = 256;
    bpg = (nx*ny+tpb-1)/tpb;
	comp_dV2it<<<bpg, tpb>>>(d_it, d_dVdt);
	cudaDeviceSynchronize();
}
...
__global__ void Euler(double *d_V, double *d_dVdt, double *d_Vnew, double *d_t){
	int k = threadIdx.x + blockIdx.x * blockDim.x;

	if(k<nx*ny){

	int j = (int)(k/nx);
	d_Vnew[k] = d_V[k+nx+2+1+2*j] + dt_max*d_dVdt[k];
...
void Forward_Euler(){
	int bpg;
        ...
        //tpb = 256;
        bpg = (nx*ny+tpb-1)/tpb;
	Euler<<<bpg, tpb>>>(d_V, d_dVdt, d_Vnew, d_t);
	cudaDeviceSynchronize();
}
__global__ void Euler2(double *d_V, double *d_dVdt, double *d_Vnew, double *d_t){
	int k = threadIdx.x + blockIdx.x * blockDim.x;

	if(k<nx*ny){

	int j = (int)(k/nx);
	d_Vnew[k] = d_V[k+nx+2+1+2*j] + (dt_max/2)*d_dVdt[k];
        d_V[k+nx+2+1+2*j] = d_Vnew[k];
...
void Forward_Euler2(){
	int bpg;
        ...
        //tpb = 256;
        bpg = (nx*ny+tpb-1)/tpb;
	Euler2<<<bpg, tpb>>>(d_V, d_dVdt, d_Vnew, d_t);
	cudaDeviceSynchronize();
}
